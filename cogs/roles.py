"""
Cog: Roles
Sistem Auto Role (saat member join) + Reaction Role berbasis Button (discord.py UI).

Arsitektur
──────────
AUTO ROLE
  - Satu atau lebih role bisa didaftarkan sebagai "auto role" per guild.
  - Saat member join, semua role tersebut langsung diberikan.
  - Data disimpan di data/roles.json → key "auto_roles".

REACTION ROLE (Button)
  - Role dikelompokkan per PANEL (kategori). Satu panel = satu pesan embed
    yang berisi tombol-tombol role.
  - Setiap panel punya: nama, deskripsi opsional, channel tujuan, dan daftar
    role + label + emoji tombol.
  - Member klik tombol → dapat role jika belum punya, lepas role jika sudah punya
    (toggle). Satu tombol = satu role.
  - Data panel disimpan di data/roles.json → key "reaction_panels".
  - Setelah bot restart, View dipasang ulang ke semua pesan panel yang ada
    (persistent view via custom_id statis).

Struktur data roles.json
────────────────────────
{
  "GUILD_ID": {
    "auto_roles": [ROLE_ID, ...],
    "reaction_panels": {
      "PANEL_ID": {
        "name": "Game",
        "description": "Pilih game favoritmu!",
        "channel_id": CHANNEL_ID,
        "message_id": MESSAGE_ID,   // null sebelum panel dikirim
        "roles": [
          {"role_id": ID, "label": "Valorant", "emoji": "🎯"},
          ...
        ]
      }
    }
